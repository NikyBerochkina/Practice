                         ...
                         int* fd_in, int* fd_out, int* fd_buf, enum TokenType* type)
{
    ...
    *end = start;
    ...
    while (*end < words->size)
    {
        ...
        if (!strcmp(words->array[*end], "||"))
        {
             if (WIFEXITED(curr_status) && !WEXITSTATUS(curr_status))
            {
            ...
            {
                // выполнение и проверка статуса завершения следующей команды
                flagCheakNextComm = 1;
            }
        ...
        if (!strcmp(words->array[*end], "&&"))
        {
            if (WIFEXITED(curr_status) && !WEXITSTATUS(curr_status))
            {
                // проверяем следующую команду
                flagCheakNextComm = 1;

            }
            else
            {
                // первая команда завершилась с ошибкой -> не проверяем следующую команду
                flagCheakNextComm = -1;
            }
...
int command_execution(struct StringArray* words)
{
    ...
    while (end != words->size)
    {
        ...
        else
        {
            ...
            // определяем статус очередной комманды
            wait(&curr_status);
            // printf("%d\n", WIFEXITED(curr_status));
        }
        start = end;
    }
    return 0;
